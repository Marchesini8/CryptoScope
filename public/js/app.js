...
const tfToApi = { '1t':'24h','10t':'24h','100t':'24h','1000t':'24h','1s':'24h','5s':'24h','10s':'24h','15s':'24h','30s':'24h','45s':'24h','1m':'24h','2m':'24h','3m':'24h','5m':'24h','10m':'24h','15m':'24h','30m':'30d','45m':'30d','1h':'30d','2h':'90d','3h':'90d','4h':'90d','6h':'180d','8h':'180d','12h':'180d','1d':'1y','3d':'1y','7d':'1y','30d':'all','1y':'all','all':'all' };
...
const tfVisible = { '1t':110,'10t':110,'100t':110,'1000t':110,'1s':110,'5s':110,'10s':110,'15s':110,'30s':110,'45s':110,'1m':110,'2m':110,'3m':110,'5m':110,'10m':110,'15m':110,'30m':110,'45m':110,'1h':110,'2h':110,'3h':110,'4h':110,'6h':110,'8h':110,'12h':110,'1d':110,'3d':110,'7d':110,'30d':110,'1y':110,'all':120 };
...
const chartState = { rows: [], volumes: [], drawings: [], currentTool: 'cursor', draft: null, freehand: null, indicator: false, tf: '5m', pad: { l: 48, r: 74, t: 24, b: 78 }, scale: null, zoom: 2.35, offset: 0, leftOffset: 0, rightOffset: 38, dragging: false, dragStartX: 0, dragStartOffset: 0, dragStartLeftOffset: 0, dragStartRightOffset: 38, crosshair: null, candleStartedAt: Date.now(), raf: 0, changeBase24h: null };
...
function visibleRows(){
  ...
  const leftWanted = Math.max(0, Math.min(totalSlots - 8, Math.round(chartState.leftOffset || 0)));
  const rightWanted = Math.max(0, Math.min(totalSlots - 8 - leftWanted, Math.round(chartState.rightOffset || 0)));
  const count = Math.min(rows.length, Math.max(8, totalSlots - leftWanted - rightWanted));
  const blankSlots = Math.max(0, totalSlots - count);
  const leftPad = Math.max(0, Math.min(blankSlots, leftWanted));
  const rightOffset = blankSlots - leftPad;
  const maxOffset = Math.max(0, rows.length - count);
  chartState.offset = Math.max(0, Math.min(maxOffset, chartState.offset));
  chartState.leftOffset = leftPad;
  chartState.rightOffset = rightOffset;
  const start = Math.max(0, rows.length - count - Math.floor(chartState.offset));
  return { rows: rows.slice(start, start + count), start, totalSlots, rightOffset, leftPad };
}
...
function setChartZoom(canvas, nextZoom, centerX){
  ...
  const anchor = before.start + ((ratio * Math.max(before.totalSlots - 1, 1)) - before.leftPad);
  chartState.zoom = Math.max(MIN_CHART_ZOOM, Math.min(MAX_CHART_ZOOM, nextZoom));
  const after = visibleRows();
  if(after.totalSlots > rows.length){
    const blankSlots = after.totalSlots - rows.length;
    chartState.leftOffset = Math.floor(blankSlots / 2);
    chartState.rightOffset = blankSlots - chartState.leftOffset;
    chartState.offset = 0;
    visibleRows();
    return;
  }
  chartState.leftOffset = 0;
  const centeredAfter = visibleRows();
  const wantedStart = anchor - ((ratio * Math.max(centeredAfter.totalSlots - 1, 1)) - centeredAfter.leftPad);
  const afterCount = centeredAfter.rows.length;
...
function panChart(canvas, deltaPixels){
  ...
  const slots = deltaPixels / perCandle;
  if(view.rows.length === chartState.rows.length && view.totalSlots > chartState.rows.length){
    chartState.leftOffset += slots;
    chartState.rightOffset -= slots;
  }else{
    chartState.offset += slots;
    chartState.leftOffset = 0;
    chartState.rightOffset -= slots;
  }
  visibleRows();
...
function drawChartTimeAxis(ctx, rows, pad, w, h, step, start, leftPad = 0){
  ...
  rows.forEach((r, i) => {
    ...
    const x = pad.l + (leftPad + i) * step + step / 2;
...
function drawChartVolumes(ctx, rows, vols, pad, h, step, maxVol, leftPad = 0){
...
function attachDrawing(canvas){
  ...
  canvas.addEventListener('pointerdown',e=>{
    ...
    if(chartState.currentTool==='cursor'){chartState.dragging=true;chartState.dragStartX=e.clientX;chartState.dragStartOffset=chartState.offset;chartState.dragStartLeftOffset=chartState.leftOffset;chartState.dragStartRightOffset=chartState.rightOffset;return;}
  ...
  canvas.addEventListener('pointermove',e=>{
    ...
    if(chartState.dragging){const dx=e.clientX-chartState.dragStartX; const view=visibleRows(); const perCandle=(canvas.parentElement.getBoundingClientRect().width-chartState.pad.l-chartState.pad.r)/Math.max(view.totalSlots,1); const slots=dx/perCandle; if(view.rows.length===chartState.rows.length&&view.totalSlots>chartState.rows.length){chartState.leftOffset=chartState.dragStartLeftOffset+slots;chartState.rightOffset=chartState.dragStartRightOffset-slots;}else{chartState.offset=chartState.dragStartOffset+slots;chartState.leftOffset=0;chartState.rightOffset=chartState.dragStartRightOffset-slots;} visibleRows(); requestChartDraw(canvas); return;}
  ...
  const finishPointer = e => {
    ...
    if(activePointers.size < 2){
      ...
      if(activePointers.size === 1 && chartState.currentTool === 'cursor'){
        ...
        chartState.dragStartLeftOffset = chartState.leftOffset;
        chartState.dragStartRightOffset = chartState.rightOffset;
        return;
      }
    }
  ...
  canvas.addEventListener('touchstart', e => {
    ...
    touchGesture = { mode: 'pan', startX: p.x, startY: p.y, offset: chartState.offset, leftOffset: chartState.leftOffset, rightOffset: chartState.rightOffset };
    chartState.dragging = true;
  }, { passive: false });
  canvas.addEventListener('touchmove', e => {
    ...
    if(e.touches.length === 1){
      ...
      const rect = canvas.parentElement.getBoundingClientRect();
      const perCandle = Math.max(1, (rect.width - chartState.pad.l - chartState.pad.r) / Math.max(view.totalSlots, 1));
      const slots = (p.x - touchGesture.startX) / perCandle;
      if(view.rows.length === chartState.rows.length && view.totalSlots > chartState.rows.length){
        chartState.leftOffset = touchGesture.leftOffset + slots;
        chartState.rightOffset = touchGesture.rightOffset - slots;
      }else{
        chartState.offset = touchGesture.offset + slots;
        chartState.leftOffset = 0;
        chartState.rightOffset = touchGesture.rightOffset - slots;
      }
      visibleRows();
...
function fearGreedText(label, value){const raw=String(label||'').toLowerCase(); if(raw.includes('extreme fear'))return 'Medo extremo'; if(raw==='fear')return 'Medo'; if(raw==='neutral')return 'Neutro'; if(raw==='greed')return 'Ganancia'; if(raw.includes('extreme greed'))return 'Ganancia extrema'; const n=Number(value||0); if(n<=24)return 'Medo extremo'; if(n<=44)return 'Medo'; if(n<=55)return 'Neutro'; if(n<=74)return 'Ganancia'; return 'Ganancia extrema';}
async function updateWatchlistFear(){try{const card=$('[data-watch-fear]'); if(!card)return; const r=await fetch('/api/market-metrics?t='+Date.now()); if(!r.ok)throw new Error('metrics failed'); const data=await r.json(); const fear=data&&data.fearGreed; if(!fear)return; const value=Math.max(0,Math.min(100,Math.round(Number(fear.value||0)))); card.style.setProperty('--value',value); card.dataset.zone=value<=44?'fear':(value<=55?'neutral':'greed'); const score=$('[data-watch-fear-score]',card), label=$('[data-watch-fear-label]',card), time=$('[data-watch-fear-time]',card); if(score)score.textContent=value; if(label)label.textContent=fearGreedText(fear.label,value); if(time&&fear.updatedAt){time.textContent='Atualizado '+new Date(fear.updatedAt).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});} }catch(_){}}
...
async function updateWatchlist(){try{const body=$('#watchlistRows'); if(!body)return; const currentId=$('#candleChart')?.dataset.coin || 'bitcoin'; const featuredIds=[currentId,'ethereum','binancecoin','tether'].filter((id,i,arr)=>id&&arr.indexOf(id)===i); const r=await fetch('/api/ranking'); if(!r.ok)throw new Error('watchlist ranking failed'); const ranking=await r.json(); const allRows=Array.isArray(ranking.usd)?ranking.usd:[]; const rows=featuredIds.map(id=>allRows.find(c=>c.id===id)).filter(Boolean); if(!rows.length)return; const html=rows.map((c,index)=>{const isCurrent=c.id===currentId; const livePrice=isCurrent?activeChartPrice():NaN; const liveChange=isCurrent?activeChartChange():NaN; const rowPrice=Number.isFinite(livePrice)?livePrice:Number(c.current_price||0); const change=Number.isFinite(liveChange)?liveChange:Number(c.price_change_percentage_24h_in_currency||0); const move=absMove(rowPrice,change); const cls=change>=0?'pos':'neg'; return '<tr class="'+(index===0?'active-watch ':'')+cls+'" data-watch="'+c.id+'" data-href="/moeda/'+c.id+'"><td><span class="watch-symbol">'+watchIconHtml(c)+'<span>'+(c.symbol||'').toUpperCase()+'USD</span></span></td><td data-watch-price>'+axisPrice(rowPrice)+'</td><td data-watch-var class="'+cls+'">'+signedNumber(move,2)+'</td><td data-watch-change class="'+cls+'">'+pct(change)+'</td></tr>';}).join('')+'<tr class="watch-more-row"><td colspan="4"><a class="watch-more" href="/watchlist">Ver mais</a></td></tr>'; if(!body.dataset.loaded){body.innerHTML=html; body.dataset.loaded='1'; body.addEventListener('click',e=>{const tr=e.target.closest('tr[data-href]'); if(tr) location.href=tr.dataset.href;}); } else {body.innerHTML=html;} syncCurrentWatchlistPrice(activeChartPrice(),activeChartChange()); }catch(_){}}
  ...
  window.addEventListener('resize',()=>chartState.rows.length&&drawCandles(canvas)); load('5m'); updateWatchlistFear(); setInterval(()=>load(chartState.tf),120000); setInterval(updateCoinPrice,2000); setInterval(updateWatchlist,5000); setInterval(updateWatchlistFear,60000); }
